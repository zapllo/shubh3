...
function ClientCard({ client, isMobile }: { client: Client; isMobile: boolean }) {
    return (
        <article className={`${isMobile
            ? 'min-w-[calc(100vw-32px)] max-w-[calc(100vw-32px)] '
...
export default function ClientsSlider() {
    ...
    return (
        <section className="w-full md:mt-12 mt-4 pt-8">
            <div className="mx-auto md:max-w-[1240px]  px-4">
                <div className="relative">
                    <h2 className="mb-20 text-center text-[22px] md:text-3xl font-bold leading-[1.6] text-[#ffffff]">
                        We Help Launched 100s of Funnels<br className='md:hidden' /> For Our Clients From<br className='hidden md:block' /> Scratch To <br className='md:hidden' /> <span className="thin-underline">Make ₹5–₹10 Lakh Every Single <br className='md:hidden' /> Week!</span>
                    </h2>

                    {/* Background blur image */}
                    <div className="absolute top-0 left-0 -z-10 transform scale-x-[-1] opacity-30">
                        <img
                ...
                <div className="relative group">
                    ...
                    <button
                        ...
                        className={`absolute left-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-white/90 p-3 shadow-lg hover:bg-white hover:shadow-xl transition-all duration-300 backdrop-blur-sm border border-gray-200 ${isMobile
                    ...
                    <button
                        ...
                        className={`absolute right-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-white/90 p-3 shadow-lg hover:bg-white hover:shadow-xl transition-all duration-300 backdrop-blur-sm border border-gray-200 ${isMobile
                            ? 'opacity-70 hover:opacity-100'
                            : 'opacity-0 md:group-hover:opacity-100'
                            }`}
                    >
                    ...
                    <div className="flex justify-center mt-6 gap-2">
                        {clients.map((_, index) => (
                            <button
                                ...
                                className={`w-2 h-2 rounded-full transition-colors duration-200 ${index === currentIndex ? 'bg-[#0F2C80]' : 'bg-gray-300'
                                    }`}
